...
- **The current focused suite has 3 skills and 4 commands.** Skills: `no-english`, `extragoal`, and `insane-review`. Commands: bare `/omg` plus `/omg:setup`, `/omg:no-english`, and `/omg:insane-review`. The presentation skill never auto-activates from ordinary natural language; only its explicit command may load it.
- **Native install is REQUIRED:** canonical command bodies remain in `templates/`; the hardened one-shot installer copies all 3 skills and 4 commands, removes explicitly retired suite-owned native surfaces and the retired private multi-harness runtime, and emits the suite-root binding.
