...
- `ouroboros` (suite-specific compatibility/setup diagnostic): requires Python >=3.12, upstream `ouroboros-ai` >=0.51.7, and a GJC build exposing `--mode rpc`. Live verification on GJC 0.14.0 fails because that transport is absent (`Malformed JSONL from GJC RPC` after plain banner output), so `/omg:ouroboros-setup` MUST report incompatibility and stop before setup rather than treating an installed bridge as working. It must never silently install/update. Do not add a plan wrapper until upstream provides a compatible runtime plus resumable dispatch and Seed gate attestations; use native `deep-interview`/`ralplan`.
