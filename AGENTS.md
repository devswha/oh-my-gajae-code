...
- **CDP↔profile binding (v0.34.1):** Chrome 136+/145+ no longer writes the `DevToolsActivePort` receipt into the user-data-dir (measured 2026-08-19 on Chrome 145.0.7632.45 — fresh headless and GUI launches leave no file), which made the receipt-only binding fail-closed on every run. The engine now proves the binding with the **loopback listener process itself** (exact `127.0.0.1`/`::1` address match; a Chromium-family executable via `/proc/pid/exe`/`ps comm`/CIM; last-value `--user-data-dir` == dedicated profile + `--remote-debugging-port` as absolute-path strings) and keeps the receipt as a secondary proof for older Chromium; the shared proof also enforces the hardened profile dir (owner/0700/no symlinks) so the `gpt_image_web` delegation keeps its original checks. `gpt_image_web.py` delegates to the shared `cdp_binds_dedicated_profile` instead of duplicating the check. Cross-reviewed (REQUEST_CHANGES round closed: loopback-address, executable-family, duplicate-flag, profile-hardening, Windows-parse issues all fixed).
