...
New installs write only `oh-my-gajae-code` bindings. The former `oh-my-gjc` suite-root binding is a read-only fallback for at least 30 days or two releases; it is never rewritten or cleaned up by this cutover. Existing old XDG research data, credentials, and `models.yml` remain in place, and the stable internal `oh-my-gjc:gate-always` marker is preserved.
...
- `adaptive-response`, `no-english`, `extragoal`, and the `example-plugin` template: no external prerequisites. `multi-harness-research` requires Linux user namespaces, executable `bwrap`, its trusted private user-scope runtime binding, supported exact credential-file layouts, and existing GJC/Codex/Claude logins; it neither installs nor logs in to a provider.
- `ouroboros` (suite-specific bridge): requires Python >=3.12, `gjc`, and externally installed upstream `ouroboros-ai` >=0.51.7. Keep it thin and explicit: `/omg:ouroboros-setup` may check install/bridge and run `ouroboros update --check`, but must never silently install or update; use `ouroboros update --yes --runtime gjc` only after user approval. Do not add a plan wrapper until upstream exposes resumable GJC dispatch with structured interview continuation and Seed client-gate attestations; use native `deep-interview`/`ralplan` meanwhile. Bare `ooo ...` belongs to upstream Ouroboros, not OMG slash commands. OMG install copies only wrapper surfaces; OMG uninstall removes only its own skill/templates and preserves the external package, `~/.ouroboros`, GJC bridge extension, MCP state, Seeds, and runs.
