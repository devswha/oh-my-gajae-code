...
- **CDP↔profile binding (v0.34.1):** Chrome 136+/145+ no longer writes the `DevToolsActivePort` receipt into the user-data-dir (measured 2026-08-19 on Chrome 145.0.7632.45 — fresh headless and GUI launches leave no file), which made the receipt-only binding fail-closed on every run. The engine now proves the binding with the **listener process's own argv** (`--user-data-dir` == dedicated profile + `--remote-debugging-port`; Linux `/proc/net/tcp*`+`/proc/*/fd`, macOS `lsof`, Windows `netstat`+CIM) and keeps the receipt as a secondary proof for older Chromium. `gpt_image_web.py` delegates to the shared `cdp_binds_dedicated_profile` instead of duplicating the check. Both proofs bind to the SAME dedicated profile; neither weakens the fail-closed contract.
